// Client-side fallback simulation engine for standalone deployment (e.g. Netlify)
import { initialStations, initialAlerts, initialResponseLogs } from '../services/seedData.js';
import { AiRiskEngine } from '../services/aiRiskEngine.js';
...
export class ClientDataStore {
  ...
  tick() {
    ...
    this.stations.forEach(station => {
      ...
      cur.waterLevel = Math.max(0.2, Math.min(maxCap, Number((cur.waterLevel + levelDelta).toFixed(2))));
      ...
      // Auto generate alerts on risk breach
    ...
    if (this.onUpdate) {
      this.onUpdate({
        ...
        alerts: this.alerts,
        responseLogs: this.responseLogs,
